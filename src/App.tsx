...
const saveAssessmentToSession = (result: DiagnosticResult, source: SavedAssessmentMeta['source'] = 'completed_assessment') => {
  ...
  try {
    ...
    console.warn('[AI Transformation] Could not save assessment recovery payload', error);
  }
};
...
const TIER1_FIXTURES: Array<{ pack_id: string; name: string; label: string; text: string }> = [
  { pack_id: 'tier1-governance-policy', name: 'tier1-governance-policy.txt', label: 'Responsible AI Governance Policy', text: tier1GovernancePolicy },
  { pack_id: 'tier1-data-ownership-standard', name: 'tier1-data-ownership-standard.txt', label: 'AI Data Ownership Standard', text: tier1DataOwnershipStandard },
  { pack_id: 'tier1-coe-charter', name: 'tier1-coe-charter.txt', label: 'AI Transformation CoE Charter', text: tier1CoeCharter },
  { pack_id: 'tier1-ai-transformation-strategy', name: 'tier1-ai-transformation-strategy.txt', label: 'AI Transformation Strategy', text: tier1AiTransformationStrategy },
  { pack_id: 'tier1-platform-lifecycle-standard', name: 'tier1-platform-lifecycle-standard.txt', label: 'AI Platform Lifecycle Control Standard', text: tier1PlatformLifecycleStandard },
            ...
            {PERSONA_IDS.map(persona => (
              <label key={persona} className="block">
                ...
                <textarea
                  ...
                  value={strategy.executive_summaries?.[persona] || (persona === 'transformation_lead' ? strategy.executive_summary || '' : '')}
                  onChange={(e) => onChange(draft => {
                    draft.phase_3_strategy.executive_summaries = {
                      ...(draft.phase_3_strategy.executive_summaries || {}),
                      [persona]: e.target.value
                    } as any;
                    if (persona === 'transformation_lead') draft.phase_3_strategy.executive_summary = e.target.value;
                  })}
                />
              </label>
            ))}
...
const App: React.FC = () => {
  ...
  const makeLowRelevanceWarning = (scan: ScanResult, kind?: UploadedFile['kind']): ScanResult => ({
    ...
    canRun: true,
    confidence_warning: scan.confidence_warning || 'This file has weak AI Transformation keyword signal. The assessment will run, but unsupported areas should be treated as insufficient evidence.',
    details: [
      ...scan.details,
      'Accepted as parseable source material; evidence gates will determine whether it supports AI Transformation findings.'
    ]
  });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (lowSignalFiles.length > 0) {
        ...
        globalScan.details.push(`${lowSignalFiles.length} file(s) have low AI Transformation keyword signal and will be assessed with evidence-gated confidence.`);
      }
    ...
    if (newFiles.length === 1) {
      ...
      if (file.type === 'text/html' || file.name.endsWith('.html')) {
        ...
        if (imported.kind === 'report') {
          ...
          setRecoveryNotice('Imported a saved AI Transformation report from HTML.');
          setError(null);
          clearFileInput();
          return;
        }
        ...
        try {
          ...
          if (imported.kind === 'report' && isDiagnosticResultPayload(imported.result)) {
            ...
            setViewMode('dashboard');
            setRecoveryNotice('Imported a saved AI Transformation report from JSON.');
            setError(null);
            clearFileInput();
            return;
          }
  ...
  const downloadResultJson = (saved: DiagnosticResult | null) => {
    ...
    a.href = url;
    a.download = `AI Transformation_Recovered_Assessment_${new Date().toISOString().split('T')[0]}.json`;
    a.click();
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setActiveTab('overview')}>
            ...
            <div className="leading-tight">
              <h1 className="text-lg font-display font-bold tracking-tight text-white group-hover:text-emerald-400 transition-colors">AI Transformation Engine</h1>
                ...
                <div className={`glass-panel rounded-[3rem] shadow-[0_0_50px_rgba(0,0,0,0.3)] border relative overflow-hidden group transition-all duration-500 ${files.length >= MIN_FILES ? 'border-emerald-500/50 ring-2 ring-emerald-500/20 shadow-[0_0_50px_rgba(16,185,129,0.1)]' : 'border-white/10'}`}>
                  ...
                  <div className="flex justify-between items-center px-10 py-6 bg-slate-900/60 backdrop-blur-xl relative z-10 border-t border-white/5">
                    <div className="flex items-center gap-4">
                      ...
                      <label
                title="Forces synthesis to use Opus 4.7 (slower, more expensive, deeper roadmap reasoning). Auto-enabled for emerging-stage organizations with high anti-pattern burden."
                    ...
                    <button onClick={handleAnalyze} disabled={!scanResult.canRun || files.length < MIN_FILES || files.length > MAX_FILES} className={`px-8 py-4 rounded-xl font-bold shadow-2xl transition-all transform active:scale-[0.98] flex items-center gap-3 border ${!scanResult.canRun || files.length < MIN_FILES || files.length > MAX_FILES ? 'bg-slate-800 text-slate-500 border-slate-700 cursor-not-allowed shadow-none' : 'text-slate-900 bg-white border-white hover:bg-emerald-400 hover:border-emerald-400 hover:shadow-[0_0_30px_rgba(16,185,129,0.4)]'}`}>
                      {!scanResult.canRun || files.length < MIN_FILES || files.length > MAX_FILES ? (
                        ...
                        <>
                          <span>Run AI Transformation Assessment</span>
          ...
          <div className="animate-fade-in space-y-12 mb-20">
              ...
              <div className="animate-fade-in space-y-8">
                  ...
                  <div className="flex items-start gap-6 mb-8">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold uppercase tracking-widest text-slate-400">
                        Classification: <span className={`${result.phase_2_validation.readiness_stage.includes('Insufficient') || result.phase_2_validation.readiness_stage.includes('Emerging') ? 'text-rose-400' : result.phase_2_validation.readiness_stage.includes('Adaptive') ? 'text-emerald-400' : 'text-amber-400'}`}>{result.phase_2_validation.readiness_stage}</span>
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="md:col-span-2 md:row-span-2">
                    <GaugeCard
                      size="large"
                      value={result.phase_2_validation.metrics.ai_readiness}
                      label="Evidence-Gated Readiness"
                      color={result.quality_gate.decision === 'BLOCK' ? '#f43f5e' : '#10b981'}
                      trend="positive"
                      subLabel={result.phase_2_validation.metrics.readiness_cap_reason ? `Capped at ${Math.round(result.phase_2_validation.metrics.readiness_cap ?? result.phase_2_validation.metrics.ai_readiness)}%` : 'Validated Index'}
                      description={result.phase_2_validation.metrics.readiness_cap_reason || METRIC_DESCRIPTIONS.ai_readiness}
                    />
                  </div>
            ...
            {activeTab === 'audit' && (
              <div className="animate-fade-in grid grid-cols-1 gap-8">
                <AuditGrid title="AI Transformation Maturity Audit" data={result.phase_1_audit_logs.maturity} isAntipattern={false} />
        ...
        <div className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-3 gap-8 items-center text-center md:text-left">
          <div className="space-y-4">
            <div className="flex items-center justify-center md:justify-start gap-3 opacity-60 hover:opacity-100 transition-opacity">
              <div className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center font-bold text-white text-xs">AI</div>
