...
import { BATCH_TITLES, MASTER_BINGO_AI } from '../knowledge_base';
...
const PERSONA_SUMMARY_LABELS = {
  transformation_lead: 'AI Transformation Lead',
  service_owner: 'CFO',
  technology_lead: 'Engineering Lead'
} as const;
...
const renderForensicSection = (
  ...
  const catalog = MASTER_BINGO_AI[stream];
...
export const downloadMasterDataReport = (result: DiagnosticResult) => {
  downloadHtml(
    ...
    `AI Transformation_Master_Data_${new Date().toISOString().split('T')[0]}.html`
  );
};
...
export const downloadSummaryReport = (result: DiagnosticResult) => {
  downloadHtml(
    ...
    `AI Transformation_Summary_Report_${new Date().toISOString().split('T')[0]}.html`
  );
};
...
const renderHeatmapCells = (
  ...
  const items = MASTER_BINGO_AI[stream].filter(c => c.batch === batch);
  return `
...
const generateSummaryReportHtml = (result: DiagnosticResult): string => {
  ...
  const readinessStage = result.phase_2_validation.readiness_stage;
  const isBlocked = result.quality_gate.decision === 'BLOCK';
  const isInsufficientEvidence = isBlocked || m.evidence_density < 30 || m.antipattern_coverage < 60;
  const readinessDescription = m.readiness_cap_reason || METRIC_DESCRIPTIONS.ai_readiness;
  const gauges = [
    { value: m.ai_readiness, label: 'Evidence-Gated Readiness', color: isBlocked ? '#f43f5e' : '#10b981', description: readinessDescription, trend: 'positive' as const, size: 'large' as const },
...
<head>
  ...
  <title>AI Transformation Summary Report</title>
  <style>
...
<body>
  <main class="page">
    <header class="hero">
      <h1>AI Transformation Summary Report</h1>
      ...
      <div class="hero-meta">
        ...
        <span class="pill">Classification ${escapeHtml(readinessStage)}</span>
    ...
    <footer class="footer">
      <p>AI Transformation Assessment Engine v${escapeHtml(result.meta.engine_version)} · Summary Report</p>
  ...
  <script id="ai-transformation-data" type="application/json">${serializeDiagnosticResultForHtml(result)}</script>
</body>
</html>`;
};
...
const generateReportHtml = (result: DiagnosticResult): string => {
  ...
  const isInsufficientEvidence = isBlocked || m.evidence_density < 30 || m.antipattern_coverage < 60;
  const readinessSlug = readinessStage.toLowerCase().includes('insufficient') || readinessStage.toLowerCase().includes('emerging') ? 'emerging' : readinessStage.toLowerCase().includes('adaptive') ? 'adaptive' : 'structured';
  const readinessDescription = m.readiness_cap_reason || METRIC_DESCRIPTIONS.ai_readiness;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>AI Transformation Maturity Assessment Report</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .classification.emerging { background: #ffe4e6; color: #be123c; }
    .classification.structured { background: #fef3c7; color: #b45309; }
    .classification.adaptive { background: #d1fae5; color: #047857; }
...
<body>
  <h1>AI Transformation Maturity Assessment</h1>
  <div class="meta">
  ...
  <div class="classification-panel">
    <div class="classification-row">
      <span class="classification ${readinessSlug}">${escapeHtml(readinessStage)}</span>
    ...
    <div class="metric-grid">
      <div class="metric">
        ...
        <div class="metric-value ${isBlocked ? 'rose' : 'emerald'}">${Math.round(m.ai_readiness)}%</div>
  ...
  ${(() => {
    const summaries = result.phase_3_strategy.executive_summaries;
    const personas: Array<{ id: 'transformation_lead' | 'service_owner' | 'technology_lead'; label: string }> = [
      { id: 'transformation_lead', label: 'AI Transformation Lead' },
      { id: 'service_owner', label: 'CFO' },
      { id: 'technology_lead', label: 'Engineering Lead' }
    ];
  ...
  ${renderForensicSection('Forensic Audit: AI Transformation Maturity', 'maturity', result.phase_1_audit_logs.maturity)}
  ...
  <div class="footer">
    <p>AI Transformation Assessment Engine v${escapeHtml(result.meta.engine_version)}</p>
  </div>
