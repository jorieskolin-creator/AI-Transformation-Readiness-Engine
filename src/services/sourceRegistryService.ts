...
const DOMAIN_TERMS: Record<string, string[]> = {
  A: [
    'operating rhythm', 'decision rights', 'demand routing', 'intake', 'capacity', 'competence',
    'launch-and-learn', 'pilot', 'learning loop', 'community', 'chapter', 'guild', 'work redesign',
    'human-in-the-loop', 'role', 'handoff', 'escalation', 'service area', 'cost profile'
  ],
  B: [
    'architecture', 'integration', 'api', 'service boundary', 'connector', 'legacy', 'lifecycle',
    'release control', 'prompt version', 'model version', 'agent', 'observability', 'evaluation',
    'drift', 'guardrail', 'red team', 'model gateway', 'routing', 'quota', 'cache', 'platform'
  ],
  C: [
    'strategy', 'purpose', 'impact statement', 'value hypothesis', 'business case', 'governance',
    'responsible ai', 'risk classification', 'oversight', 'portfolio', 'investment', 'budget',
    'forecast', 'guardrail', 'unit economics', 'cost per', 'roi', 'tvo', 'kill', 'continue', 'scale'
  ],
  D: [
    'data ownership', 'data product', 'semantic', 'context', 'lineage', 'freshness', 'quality',
    'access', 'privacy', 'classification', 'masking', 'retrieval', 'rag', 'embedding', 'vector',
    'feature store', 'source coverage', 'grounding', 'knowledge store'
  ],
  E: [
    'service catalog', 'capability map', 'business capability', 'service blueprint', 'value stream',
    'customer journey', 'frontstage', 'backstage', 'solution structure', 'traceability',
    'service area team', 'ownership', 'rollout', 'scaling', 'cost-to-serve', 'customer outcome'
  ]
};
...
export const buildSourceRegistry = (text: string, images: ImageInput[] = []): SourceRegistry => {
  ...
  docs.forEach((doc, docIndex) => {
    ...
    for (const page of pages) {
      ...
      if (isTable) {
        chunks.push({
          chunk_id: chunkIdFor(sourceId, chunkIndex, page.pageNumber),
          source_id: sourceId,
...
const tierForDomain = (chunk: SourceChunk, domain: string): SourceRelevanceTier => (
  chunk.routing.find(r => r.domain === domain)?.tier || 'unknown'
);

const scoreForDomain = (chunk: SourceChunk, domain: string): number => (
  chunk.routing.find(r => r.domain === domain)?.score || 0
);
...
export const scanRegistryDlp = (registry: SourceRegistry): DlpScanResult => {
  const patterns: Array<{ kind: DlpPatternHit['kind']; severity: DlpPatternHit['severity']; rx: RegExp }> = [
    ...
    { kind: 'financial_caution', severity: 'caution', rx: /\b(?:contract value|discount rate|edp pricing|negotiated rate|billing account|invoice number|token spend|model spend|usage cost)\b/gi }
  ];
...
const hasDlpRiskText = (chunk: SourceChunk): boolean => {
  return /\[EMAIL_REDACTED\]|\[PHONE_REDACTED\]|\[IP_REDACTED\]|\[AWS_KEY_REDACTED\]|\[API_KEY_REDACTED\]|AKIA[0-9A-Z]{16}|sk-[a-zA-Z0-9]{20,}|PRIVATE KEY|billing account|contract value|discount rate|edp pricing|token spend|model spend/i.test(chunk.text);
};
