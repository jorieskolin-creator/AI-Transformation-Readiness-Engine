...
import { knowledgeBaseService, BATCH_DEFINITIONS, AI_TACTICS_LOCAL, AI_TACTIC_ACTIVITY_PLAYBOOK, AI_TAXONOMY_REGISTRY, buildTacticIdTable, validTacticIdSet } from "../knowledge_base";
...
These are not valid. You either invented them, abbreviated a real ID (e.g. TAC-OPER- vs TAC-OPS-, TAC-ARCH- vs TAC-PLAT-), or appended a suffix (e.g. -COM) that does not exist.
...
const DEFAULT_PERSONA: PersonaId = 'transformation_lead';
...
const normalizePersonaSummaries = (rawStrategy: any): {
  ...
  const result: Record<PersonaId, string> = { transformation_lead: '', service_owner: '', technology_lead: '' };
...
const parseAiResponse = (text: string): any => {
  ...
  if (!jsonMatch) {
    console.warn("[AI Transformation Pipeline] AI Response contained no JSON braces.");
    return {};
  }
  try {
    return JSON.parse(jsonMatch[0]);
  } catch (e) {
    console.error("[AI Transformation Pipeline] JSON Parse Error:", text.substring(0, 500));
...
export const analyzeDocument = async (
  ...
  console.log(`[AI Transformation] === Pipeline start === run=${runId} deepMode=${!!options.deepMode}`);
  ...
  try {
    ...
    console.log("[AI Transformation] DLP Scan Passed.");

    console.log("[AI Transformation] Pre-fetching Tactics Database for Phase 3...");
    ...
    onProgress('audit', 5);
    console.log(`[AI Transformation] [${runId}] Running Phase 1 Parallel Audit (5 batches)...`);
    ...
    if (phase1Validation.warnings.length > 0) {
      console.warn("[AI Transformation] Phase 1 validation warnings:", phase1Validation.warnings);
    }
    ...
    console.log(`[AI Transformation] Phase 2 Complete. Readiness: ${Math.round(validationData.metrics.ai_readiness)}%, Classification: ${validationData.readiness_stage}`);
    ...
    console.log(`[AI Transformation] [${runId}] Synthesis confidence: ${bracketDetail}`);
    ...
    const escalationReason = lowConfidenceFastPath
      ...
      : autoEscalate
        ...
        : 'none';
    console.log(`[AI Transformation] [${runId}] Synthesis stage: ${synthesisStage} (${escalationReason})`);
    serverLog(runId, 'info', 'synthesis_routing', {
      stage: synthesisStage,
      reason: escalationReason,
      readiness: Math.round(validationData.metrics.ai_readiness),
      ...
      class: validationData.readiness_stage,
    });
...
AI Transformation DIAGNOSTIC REPORT SUMMARY (Computed by System):
-------------------------------------------------------
Evidence-Gated AI Transformation Readiness Score: ${Math.round(validationData.metrics.ai_readiness)}/100
Maturity Classification: ${validationData.readiness_stage}
...
`;
    ...
    const runFactCheck = async (data: any, attemptNumber: number, stage: Extract<StageId, 'fact_check' | 'fact_check_high'> = 'fact_check'): Promise<FactCheckResult> => {
      ...
      try {
        ...
        const roadmapPrompt = buildRoadmapFactCheckPrompt({
          ...
          tacticActivityPlaybook: AI_TACTIC_ACTIVITY_PLAYBOOK,
        });
    ...
    const buildFallbackEvidenceSummary = () => ({
      headline: `${validationData.readiness_stage} AI Transformation maturity with ${Math.round(validationData.metrics.ai_readiness)}/100 evidence-gated readiness`,
      maturity_classification: validationData.readiness_stage,
      key_metrics: [
        `Evidence-gated readiness: ${Math.round(validationData.metrics.ai_readiness)}/100`,
    ...
    const buildFallbackFindingsMode = () => ({
      ...
      validation_plan: [
        'Provide source material that documents current AI Transformation ownership, cadence, and decision rights.',
        'Attach evidence of AI architecture, data ownership, governance controls, platform lifecycle, and service-area accountability.',
        'Include recent AI value reviews, pilot learning records, lifecycle evidence, or service-owner decision records before rerunning the assessment.'
      ]
    });
    ...
    if (factCheck.failed) {
      console.warn(`[AI Transformation] Fact-check unavailable: ${factCheck.failure_reason}`);
    } else {
      console.log(`[AI Transformation] Fact-check complete after ${factCheck.attempts} pass(es): ${factCheck.supported_count}/${factCheck.total_claims} claims supported, ${lastUnsupported.length} unsupported.`);
      if (trajectory.length > 1) {
        const traj = trajectory.map(p => `pass${p.attempt}:${p.supported_count}/${p.total_claims}supp,${p.unsupported_count}unsupp`).join(' → ');
        console.log(`[AI Transformation] [${runId}] Fact-check trajectory: ${traj}`);
    ...
    if (groundingValidation.errors.length > 0) {
      console.error("[AI Transformation] Phase 3 grounding errors:", groundingValidation.errors);
    }
    if (groundingValidation.warnings.length > 0) {
      console.warn("[AI Transformation] Phase 3 grounding warnings:", groundingValidation.warnings);
    }
    ...
    console.log(`[AI Transformation] [${runId}] Quality Gate decision: ${qualityGate.decision}`);
    ...
    if (effectiveBracket !== confidenceBracket) {
      console.warn(`[AI Transformation] [${runId}] Strategy downgraded by QG: ${confidenceBracket} → ${effectiveBracket} (decision=${qualityGate.decision})`);
    ...
    console.log(`[AI Transformation] [${runId}] === Pipeline complete === duration_ms=${totalDuration} quality_gate=${qualityGate.decision} bracket=${effectiveBracket}`);
    ...
    const rawResult: DiagnosticResult = {
      ...
      phase_3_strategy: strategyData.phase_3_strategy || {
        ...
        executive_summaries: {
          transformation_lead: "Strategy incomplete.",
          service_owner: "Strategy incomplete.",
          technology_lead: "Strategy incomplete."
        },
        active_persona: DEFAULT_PERSONA,
    ...
    console.error(`[AI Transformation] [${runId}] === Pipeline FAILED === duration_ms=${duration} error="${error?.message || error}"`);
