...
const VALID_SOURCE_LOCATIONS: ClaimSourceLocation[] = ['transformation_lead', 'service_owner', 'technology_lead', 'diagnosis', 'planning_decision', 'roadmap'];
...
export interface FactCheckInputs {
  ...
  // a KB-backed tactic such as "Implement [TAC-OPS-001]") are verified
...
const compactMetrics = (phase2: Phase2Validation): string => {
  ...
  const categoryScoreLines = Object.entries(phase2.category_scores || {})
    ...
    .join('\n');
  return [
    `ai_readiness=${Math.round(m.ai_readiness)}%`,
    ...
    `classification=${phase2.readiness_stage}`,
...
- "supported_by_tactics_db": the claim references a tactic ID (e.g. "TAC-OPS-001"), mechanism, or named pattern that EXISTS in the VERIFIED_TACTICS_DB section below. This is a legitimate prescription pattern, NOT a hallucination. The synthesis step is explicitly instructed to use this database — these references are sanctioned.
  ...
  Named companies, organizations, document titles, filenames, source labels, authors, and "according to the Knowledge Base" provenance are NOT sanctioned. Internal KB names are never allowed. Customer source company, organization, legal-entity, customer, vendor, and brand names are also not allowed in generated report text. Even when they appear in the customer source, classify generated-output reuse of those names as unsupported with failure_type "unverifiable_entity" so the sanitation layer can rewrite it into fluent neutral language. Placeholder tokens such as [ORGANIZATION_REDACTED] are also not acceptable customer-facing prose.
...
- If generated report text repeats a company name, organization name, legal-entity name, customer name, vendor name, or brand name from the input material, classify that name-bearing claim as unsupported with failure_type "unverifiable_entity". Use fluent neutral functional labels instead, never placeholder tokens.
...
- For every claim classified "unsupported", you MUST additionally emit:
  ...
  - "missing_material": one short sentence describing what specific evidence in a future source document would make this claim supportable (e.g., "an AI governance decision record", "a service-area value review note", "a named AI Transformation team charter").
  ...
  "claims": [
    {
      ...
      "source_location": "transformation_lead | service_owner | technology_lead | diagnosis | planning_decision | roadmap",
...
const FAILURE_TYPE_GUIDANCE: Record<ClaimFailureType, string> = {
  ...
  unverifiable_entity: 'You named a tool, team, company, product, KB document, source label, organization, customer, vendor, or brand that is not allowed in generated report text. Tactic IDs and mechanism names can be used when valid, but internal KB names and customer-source organization names are never customer-facing. Rewrite the sentence fluently with neutral language such as "the deployment pipeline", "the central team", "the assessed organization", "the AI governance process", or "internal reference material". Do not use placeholder tokens.',
...
- If the output repeats company names, organization names, legal-entity names, customer names, vendor names, or brand names from the customer source, classify the claim as unsupported with failure_type "unverifiable_entity" even if the name appears in the source. Generated summaries must be written as fluent anonymized prose, not placeholder-token prose.
  ...
  "claims": [
    {
      ...
      "rationale": "one short sentence",
      "source_location": "transformation_lead | service_owner | technology_lead | diagnosis",
...
- Internal KB document titles, filenames, source labels, authors, company names, organization names, and "according to the Knowledge Base" provenance are not customer-facing support. Customer source company, organization, legal-entity, customer, vendor, and brand names are also not allowed in generated planning or roadmap text.
- Company names, organization names, legal-entity names, customer names, vendor names, or brand names from the customer source are not allowed in planning or roadmap text. Placeholder tokens such as [ORGANIZATION_REDACTED] are also not acceptable. Flag them as unsupported with failure_type "unverifiable_entity" even if the name appears in the source.
