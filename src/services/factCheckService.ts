...
- If generated report text repeats a company name, organization name, legal-entity name, customer name, vendor name, or brand name from the input material, classify that name-bearing claim as unsupported with failure_type "unverifiable_entity". Use neutral functional labels instead.
...
const FAILURE_TYPE_GUIDANCE: Record<ClaimFailureType, string> = {
  ...
  unverifiable_entity: 'You named a tool, team, company, product, KB document, source label, organization, customer, vendor, or brand that is not allowed in generated report text. Tactic IDs and mechanism names can be used when valid, but internal KB names and customer-source organization names are never customer-facing. Remove the entity, reference it generically ("the deployment pipeline", "the central team", "the assessed organization", "internal reference material"), or replace with a verified tactic ID/mechanism from the database.',
...
- If the output repeats company names, organization names, legal-entity names, customer names, vendor names, or brand names from the customer source, classify the claim as unsupported with failure_type "unverifiable_entity" even if the name appears in the source. Generated summaries must stay anonymized.
...
- Company names, organization names, legal-entity names, customer names, vendor names, or brand names from the customer source are not allowed in planning or roadmap text. Flag them as unsupported with failure_type "unverifiable_entity" even if the name appears in the source.
