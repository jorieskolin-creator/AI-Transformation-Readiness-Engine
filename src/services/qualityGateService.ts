...
const QG_PROMPT_PREAMBLE = `You are a senior AI Transformation reviewer. A deterministic Quality Gate has flagged issues with this assessment. Your job is to explain WHY each blocker / warning matters in plain English, and where possible, anchor the explanation in a direct quote from the source document.

Return STRICT JSON in this shape:
{
  "summary": "2-3 sentences in plain language: what's wrong with this assessment and what the reader should do.",
  "blocking_details": [
    { "reason": "<verbatim text of the blocking reason>", "explanation": "1-2 sentences why this matters for the AI Transformation maturity reading", "quote": "<short quote from source if relevant, else omit>", "source_location": "<section name / page / 'unknown'>" }
  ],
  "warning_details": [
...
- Write fluent anonymized prose. Do not output company names, organization names, legal-entity names, customer names, vendor names, brand names, source-origin organization labels, or placeholder tokens such as [ORGANIZATION_REDACTED]. Use "the assessed organization" or functional labels.
- If a PDF has zero extractable text but rendered page images are available, call it visual-only or image-heavy source material. Do not say it contributed no assessable evidence, was fully ignored, or was entirely unknown; say confidence depends on visual interpretation.
