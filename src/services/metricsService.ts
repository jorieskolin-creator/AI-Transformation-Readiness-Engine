...
export const calculateMetrics = (logs: Phase1AuditLogs): Phase2Validation => {
  ...
  const ai_readiness = clampPercent(Math.min(uncapped_readiness, evidenceCap.cap));

  let readiness_stage: Phase2Validation['readiness_stage'];
  if (evidence_density < EVIDENCE_DENSITY_BLOCK) {
    readiness_stage = 'Insufficient evidence';
  } else if (ai_readiness < 33) {
    readiness_stage = 'Emerging';
  } else if (ai_readiness < 66) {
    readiness_stage = antipattern_burden > 50 ? 'Scaling with friction' : 'Structured';
  } else {
    readiness_stage = 'Adaptive / Value-creating';
  }
