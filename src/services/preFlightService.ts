...
import { AI_KEYWORDS } from '../knowledge_base';
...
export const scanInputText = (text: string): ScanResult => {
  ...
  const categories = AI_KEYWORDS.categories;

  for (const keyword of categories.core_aiTransformation_vocabulary.keywords) {
    if (lowerText.includes(keyword)) {
      uniqueKeywords.add(keyword);
      weightedScore += categories.core_aiTransformation_vocabulary.weight;
    }
  }
  ...
  for (const header of AI_KEYWORDS.structural_headers) {
  ...
  const coreCount = categories.core_aiTransformation_vocabulary.keywords.filter(k => lowerText.includes(k)).length;
  ...
  if (score >= 60 && hasCoreTerm) {
    status = 'Ready';
    message = "High Quality AI Transformation Signal";
    details.push(`Detected ${uniqueKeywords.size} AI Transformation-relevant topics (${coreCount} core terms)`);
    ...
    details.push("Document appears irrelevant to AI Transformation (e.g., unrelated business content, personal data, or generic text).");
    canRun = false;
  }
