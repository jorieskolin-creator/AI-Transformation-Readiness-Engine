...
import criteriaData from './aiTransformation_criteria.json';
import antipatternData from './aiTransformation_antipatterns.json';
import keywordsData from './aiTransformation_preflight_keywords.json';
import taxonomyData from './aiTransformation_evidence_taxonomy.json';
import personasData from './aiTransformation_personas.json';
import tacticsData from './aiTransformation_tactics_database.json';
import tacticActivityPlaybookData from './aiTransformation_tactic_activity_playbook.json';
import validationData from './aiTransformation_validation_rules.json';
import taxonomyRegistryData from './aiTransformation_taxonomy_registry.json';

export const AI_CRITERIA = criteriaData.criteria;
export const AI_ANTIPATTERNS = antipatternData.criteria;
export const AI_KEYWORDS = keywordsData;
export const AI_EVIDENCE_TAXONOMY = taxonomyData;
export const AI_PERSONAS = personasData;
export const AI_TACTICS_LOCAL = tacticsData.tactics as StrategicTactic[];
export const AI_TACTIC_ACTIVITY_PLAYBOOK = tacticActivityPlaybookData.entries as TacticActivityPlaybookEntry[];
...
export function validTacticIdSet(tactics: StrategicTactic[] = AI_TACTICS_LOCAL): Set<string> {
  return new Set(tactics.map(t => t.id));
}

export function buildTacticActivityContext(
  tactics: StrategicTactic[] = AI_TACTICS_LOCAL,
  entries: TacticActivityPlaybookEntry[] = AI_TACTIC_ACTIVITY_PLAYBOOK
): string {
...
const MASTER_BINGO_AI = {
  maturity: AI_CRITERIA.map(c => ({
    id: c.id,
    batch: c.batch,
    title: c.title,
    desc: c.description
  })),
  antipattern: AI_ANTIPATTERNS.map(c => ({
    id: c.id,
    batch: c.batch,
    title: c.title,
    desc: c.description
...
export { MASTER_BINGO_AI };

const buildBatchXml = (items: typeof AI_CRITERIA, batchId: string): string => {
  return items
...
export const TAXONOMY_DOCUMENT_NAMING = AI_TAXONOMY_REGISTRY.kb_document_naming;
export const TAXONOMY_USAGE_BOUNDARIES = AI_TAXONOMY_REGISTRY.usage_boundaries;
...
for (const batchId of ['A', 'B', 'C', 'D', 'E']) {
  BATCH_DEFINITIONS[batchId] = {
    ...
    maturity: buildBatchXml(AI_CRITERIA, batchId),
    antipattern: buildBatchXml(AI_ANTIPATTERNS, batchId)
  };
}
...
<task>
You are a **AI Transformation Evidence Auditor**. Your job is to extract **explicit proof** from the text.
...
2. **No Inference:** If the text says "We plan to define AI governance", that is NOT evidence of a functioning governance system. Score 1 max.
...
*   **AI strategy document:** Expect evidence at score 1 unless it shows operating cadence, decision records, portfolio movement, or measured outcomes. Strategy documents describe intent, not embedded operation.
*   **Responsible AI / governance policy:** Expect evidence in C3-C4 and B4 when controls, risk classes, approvals, and auditability are explicit. Expect SILENCE on delivery flow, data ownership, and service architecture unless those are documented.
*   **AI platform architecture document:** Expect evidence in B1-B5 and D5 when lifecycle, integration, observability, guardrails, and reusable platform capabilities are described. Expect SILENCE on business value unless linked to service outcomes.
*   **Data governance / data product document:** Expect evidence in D1-D5 when ownership, meaning, lineage, freshness, access, retrieval, or semantic context are described.
*   **Service blueprint / business architecture document:** Expect evidence in E1-E5 when services, capabilities, value streams, handoffs, dependencies, ownership, and rollout sequencing are described.
*   **Pilot report / experiment review:** Expect evidence in A3, C2, C5, and E5 only when learning, value hypotheses, controls, adoption, and scale decisions are documented. A pilot alone is not proof of readiness.
*   **Dashboard screenshot (image input):** Evidence only what is visible: portfolio flow, AI observability, value metrics, safety thresholds, data freshness, or adoption signals. Expect SILENCE elsewhere.
*   **Architecture diagram (image input):** May evidence B1-B5, D5, or E3 when integration, release, monitoring, retrieval, security, or traceability is visible. Expect SILENCE on operating model and value unless annotated.
*   **Org chart (image input):** Evidences accountability only when AI, data, risk, platform, or service-area roles and decision relationships are visible.
...
const FALLBACK_TACTICS: StrategicTactic[] = [
  {
    id: "TAC-FALLBACK",
    category: "AI Value Creation",
    problem_pattern: "Missing evidence of service-owned AI value creation",
    solution_mechanism: "Run a bounded Launch-and-Learn slice with explicit ownership, AI controls, data readiness, and value measurement.",
    case_study: "Reference pattern: service-area team validates an AI-enabled workflow before scaling."
  }
];
...
export const knowledgeBaseService = {
    ...
    remoteKbIndexPromise = (async () => {
      try {
        ...
        if (index.status?.source === 'remote_blob') {
          console.info(`[AI Transformation KnowledgeBase] Remote PDF KB loaded: ${index.status.document_count} documents.`);
        } else {
          console.info(`[AI Transformation KnowledgeBase] Using built-in KB fallback (${index.failures?.[0]?.reason || 'remote unavailable'}).`);
        }
        return index;
      } catch (error: any) {
        console.warn('[AI Transformation KnowledgeBase] Remote PDF KB unavailable, using built-in fallback:', error);
  ...
  async fetchStrategicPlaybook(): Promise<string> {
    ...
    if (meta?.env?.VITE_AI_TACTICS_URL) {
      blobUrl = meta.env.VITE_AI_TACTICS_URL;
    }

    if (!blobUrl) {
      try {
        // @ts-ignore
        if (typeof process !== 'undefined' && process.env?.VITE_AI_TACTICS_URL) {
          // @ts-ignore
          blobUrl = process.env.VITE_AI_TACTICS_URL;
        }
      } catch (e) {}
    }
    ...
    if (blobUrl) {
      try {
        console.log(`[AI Transformation KnowledgeBase] Fetching tactics DB from Vercel Blob...`);
        ...
        console.error("[AI Transformation KnowledgeBase] Remote fetch failed, using local DB:", error);
        tactics = AI_TACTICS_LOCAL;
      }
    } else {
      console.info("[AI Transformation KnowledgeBase] No remote URL configured. Using built-in tactics database.");
      tactics = AI_TACTICS_LOCAL;
    }
