...
import { BATCH_DEFINITIONS, MASTER_BINGO_AI } from '../knowledge_base';
        ...
        <div className="flex justify-center mb-8">
          <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-slate-800/50 border border-slate-700/50 shadow-sm backdrop-blur-sm">
            ...
            <span className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300">
              {stage === 'strategy' ? 'PHASE 3: STRATEGIC SYNTHESIS (Opus 4.7)' : 'PHASE 1: FORENSIC AUDIT (Claude Sonnet)'}
            </span>
          </div>
        </div>
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-display font-bold text-white mb-3 tracking-tight">
            {stage === 'strategy' ? 'Synthesizing Roadmap...' : 'Analyzing AI Transformation Streams...'}
          </h2>
          <p className="text-slate-300 font-medium text-lg">
            {stage === 'strategy' ? 'Claude Opus 4.7 - Strategy Engine Active' : 'Claude Sonnet - Parallel Batching Active'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 px-4 md:px-12 relative">
          <div className="relative">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-300">AI Transformation Knowledge Base</span>
            </div>
...
export const TransferProtocol: React.FC = () => (
  <div className="mt-12 mb-12 py-10 relative">
    ...
    <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-8 flex items-center justify-center gap-2 font-mono">
      ...
      AI Transformation Action Protocol
    </h4>
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative max-w-4xl mx-auto">
      ...
      {[
        ...
        { id: 3, title: "Plan Roadmap", desc: "Ready-and-Adapt" },
...
export const BenchmarkingChart: React.FC<BenchmarkingProps> = ({ x, y, evidenceDensity = 100, antipatternCoverage = 100, qualityGateDecision }) => {
  ...
  const quadrants = [
    { id: 'q1', label: 'Readiness Blindness', sub: 'Tool Adoption Without System', desc: 'High anti-pattern burden with low maturity. AI activity is visible, but operating model, data, governance, and service ownership are not yet reliable.', position: 'top-0 left-0', style: 'bg-gradient-to-br from-rose-950/30 via-slate-900/50 to-transparent border-r border-b border-white/5', text: 'text-rose-400' },
    { id: 'q2', label: 'AI Transformation Theater', sub: 'Process Without Outcomes', desc: 'Some maturity exists but anti-patterns persist. AI transformation meetings happen, but value, safety, learning, and scaling outcomes remain weak.', position: 'top-0 right-0', style: 'bg-gradient-to-bl from-amber-950/30 via-slate-900/50 to-transparent border-b border-white/5', text: 'text-amber-400' },
  ...
  return (
    <div className="p-8 rounded-[2.5rem] relative flex flex-col h-full min-h-[550px] bg-slate-900/70 border border-white/10 shadow-lg backdrop-blur-sm">
      <div className="flex justify-between items-start mb-2 relative z-20 pointer-events-none">
        <div>
          <h3 className="text-white font-display font-bold text-2xl tracking-tight pointer-events-auto">AI Transformation Maturity Matrix</h3>
          ...
          {topOffenders.map((item) => (
            <div key={item.id} className={`p-5 rounded-2xl border ${isAntipattern ? 'bg-rose-950/20 border-rose-900/30' : 'bg-emerald-950/20 border-emerald-900/30'}`}>
              ...
              <div className={`flex items-start gap-2 text-xs font-medium p-3 rounded-lg ${isAntipattern ? 'text-rose-300 bg-rose-900/20' : 'text-emerald-300 bg-emerald-900/20'}`}>
                ...
                <span><strong>Action:</strong> {isAntipattern ? `Address ${item.title.toLowerCase()} to reduce anti-pattern burden.` : `Implement ${item.title.toLowerCase()} to advance AI Transformation maturity.`}</span>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="max-w-[90rem] mx-auto animate-fade-in pb-32">
      <div className="text-center mb-12 relative z-10 px-4">
        ...
        <h2 className="text-5xl md:text-7xl font-display font-black text-white mb-6 tracking-tight drop-shadow-lg">AI Transformation Forensic Lens</h2>
        <p className="text-xl text-slate-300 max-w-3xl mx-auto leading-relaxed font-light">
          The specific behavioral signals used by the AI to detect <strong className="text-emerald-400 font-bold">AI Transformation Maturity</strong> vs. <strong className="text-rose-400 font-bold">Anti-Patterns</strong>.
        </p>
      </div>
      ...
      {dataset === 'criteria' ? (
        ...
        <div className="px-6 md:px-12 animate-fade-in-up">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {(activeStream === 'maturity' ? MASTER_BINGO_AI.maturity : MASTER_BINGO_AI.antipattern).map((item) => (
