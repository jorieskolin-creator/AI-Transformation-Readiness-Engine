...
export const ReportView: React.FC<ReportViewProps> = ({ result, onBack, onDownloadSummary, onDownloadMaster }) => {
  ...
  const readinessStage = result.phase_2_validation.readiness_stage || 'Insufficient evidence';
  ...
  const gauges = [
    { value: m.ai_readiness, label: 'Evidence-Gated Readiness', color: readinessColor, description: readinessDescription, trend: 'positive' as const, size: 'large' as const },
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-display font-bold text-slate-900 mb-2">AI Transformation Maturity Assessment</h1>
        ...
        <div className="mb-12 p-8 bg-slate-50 rounded-2xl border border-slate-200">
          <div className="flex items-center gap-4 mb-6">
            <span className={`px-4 py-2 rounded-lg font-bold text-sm ${readinessStage.includes('Insufficient') || readinessStage.includes('Emerging') ? 'bg-rose-100 text-rose-700' : readinessStage.includes('Adaptive') ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
              {readinessStage}
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className={`text-3xl font-bold ${isBlocked ? 'text-rose-600' : 'text-emerald-600'}`}>{Math.round(m.ai_readiness)}%</p>
        ...
        <div className="mb-12">
          ...
          {(() => {
            ...
            const ids: PersonaId[] = ['transformation_lead', 'service_owner', 'technology_lead'];
        ...
        <ForensicSection
          title="Forensic Audit: AI Transformation Maturity"
          stream="maturity"
        ...
        <div className="text-center py-8 border-t border-slate-200 text-sm text-slate-400">
          <p>AI Transformation Assessment Engine v{result.meta.engine_version}</p>
          <p>
