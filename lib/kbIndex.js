...
export const ROOT_PREFIX_SENTINEL = '__ROOT__';
...
const DEFAULT_ALLOWED_USES = [
  ...
  'roadmap_pattern_inspiration',
  'abstract_theme_interpretation',
];
const DEFAULT_FORBIDDEN_USES = REQUIRED_FORBIDDEN_USES;

export function normalizeKbBlobPrefix(value, defaultPrefix = 'Knowledge Base/') {
  const raw = String(value || defaultPrefix).trim();
  if (raw === ROOT_PREFIX_SENTINEL) {
    return {
      requestPrefix: '',
      displayPrefix: '(root)',
      isRoot: true,
  ...
  const requestPrefix = raw.endsWith('/') ? raw : `${raw}/`;
  return {
    requestPrefix,
    displayPrefix: requestPrefix,
    isRoot: false,
  };
}
...
function titleFromPathname(pathname) {
  const filename = String(pathname || '').split('/').pop() || 'Knowledge Base Document';
  return filename
    .replace(/\.[^.]+$/, '')
    .replace(/[_-]+/g, ' ')
    .replace(/\s{2,}/g, ' ')
...
export function sanitizeKbDocument({ pathname, url, downloadUrl, size, uploadedAt, text }) {
  ...
  try {
    const candidate = extractJsonFrontMatter(text);
    const problems = validateKbMetadata(candidate, pathname);
    if (problems.length === 0) {
      meta = candidate;
      structured = true;
    }
  } catch {
    // Simple AI Transformation KB documents do not need JSON front matter.
  }

  if (!structured) {
    const body = compactText(text, 6000);
    return {
      pathname,
      url,
      downloadUrl,
      ...
      title: firstContentLine(body) || titleFromPathname(pathname),
      evidence_categories: [],
      allowed_uses: DEFAULT_ALLOWED_USES,
      forbidden_uses: DEFAULT_FORBIDDEN_USES,
      legacy_ids: [],
      body_excerpt: body,
    };
  }
