...
const dir = await mkdtemp(join(tmpdir(), 'aiTransformation-antipattern-semantics-'));
...
assert.equal(
  resolveAntiPatternAbsenceStatus({
    ...
    evidenceStatus: 'weak',
    rationale: 'The cited material supports partial presence of black-box AI operations, but not a full score.',
  }),
  'partially_present',
...
assert.equal(
  inferAntiPatternAbsenceStatus({
    ...
    reasoning: 'Final anti-pattern assessment: Tested absent. Verifier status: weak. The source supports partial presence of pilot purgatory behavior.',
  }),
  'partially_present',
...
assert.equal(
  inferAntiPatternAbsenceStatus({
    count: 0,
    antipattern_absence_status: 'tested_absent',
    reasoning: 'Coverage notes say there is partial use-case chasing in portfolio operations.',
  }),
  'partially_present',
...
const metrics = calculateMetrics({
  maturity: {},
    ...
    D2: {
      id: 'D2',
      criterion: 'Raw Data Without Meaning',
      count: 0,
      status: 'OK',
      evidence: 'Evidence-check found a partial raw-data-without-meaning signal.',
