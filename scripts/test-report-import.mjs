...
const dir = await mkdtemp(join(tmpdir(), 'aiTransformation-report-import-'));
...
const payload = {
  ...
  phase_2_validation: { metrics: {}, readiness_stage: 'Adaptive / Value-creating' },
...
const html = `<!doctype html><script id="ai-transformation-data" type="application/json">${serialized}</script>`;
...
const brokenHtml = '<script id="ai-transformation-data" type="application/json">{not json</script>';
