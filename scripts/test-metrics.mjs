...
const dir = await mkdtemp(join(tmpdir(), 'aiTransformation-metrics-'));
...

{
  ...
  assert.equal(result.readiness_stage, 'Insufficient evidence');
  assert.ok(result.metrics.ai_readiness < 30, `expected capped low readiness, got ${result.metrics.ai_readiness}`);
...

{
  ...
  assert.equal(result.readiness_stage, 'Adaptive / Value-creating');
  assert.ok(result.metrics.ai_readiness >= 90, `expected high readiness, got ${result.metrics.ai_readiness}`);
...

{
  ...
  assert.ok(result.metrics.ai_readiness < result.metrics.maturity_depth, 'confirmed burden should reduce readiness');
