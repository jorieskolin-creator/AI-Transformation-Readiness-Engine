...
const dir = await mkdtemp(join(tmpdir(), 'AI Transformation-strategy-sanitation-'));
...
const strategyData = {
  phase_3_strategy: {
    executive_summaries: {
      service_owner: 'The anti-pattern burden is confirmed at 7%, meaning a material but bounded share of AI investment is affected by identified inefficiency patterns. Governance is otherwise documented.'
    },
    diagnosis: {
    ...
    remediation_roadmap: [
      {
        phase: '1. Emerging',
        why: 'Implement AI Transformation outcome tracking to shift measurement from documented activities toward quantified value outcomes.',
        what: 'Enforce agent behavioral boundary testing for agent tool access while preserving validated operational controls.',
        actions: [
          'Implement AI Transformation outcome tracking to shift measurement from documented activities toward quantified value outcomes.',
          'Enforce agent behavioral boundary testing for agent tool access.'
        ]
      }
    ]
  }
...
const factCheck = {
  ...
  unsupported_claims: [
    {
      claim: 'The anti-pattern burden is confirmed at 7%, meaning a material but bounded share of AI investment is affected by identified inefficiency patterns.',
      classification: 'unsupported',
      source_location: 'service_owner',
      failure_type: 'fabricated_number',
      severity: 'BLOCKING_UNSUPPORTED_FACT',
      rationale: 'Phase 2 defines a 7% anti-pattern burden but does not state that this percentage represents share of AI investment.'
    },
    {
      claim: 'Implement AI Transformation outcome tracking to shift measurement from documented activities toward quantified value outcomes.',
      classification: 'unsupported',
      source_location: 'roadmap',
      failure_type: 'unsupported_org_claim',
      severity: 'BLOCKING_UNSUPPORTED_FACT',
      rationale: 'The locked findings already include quantified value outcomes.'
    },
    {
      claim: 'AI value evidence is incomplete — only two services measured.',
      classification: 'unsupported',
...
const hygieneStrategyData = {
  phase_3_strategy: {
    executive_summaries: {
      service_owner: 'The confirmed anti-pattern is low-severity anomaly escalation, and every confirmed gap maps to exact verified tactics database patterns.'
    },
    diagnosis: {
...
assert.match(sanitized.strategyData.phase_3_strategy.executive_summaries.service_owner, /burden index is 7%/);
assert.doesNotMatch(sanitized.strategyData.phase_3_strategy.executive_summaries.service_owner, /share of AI investment/);
